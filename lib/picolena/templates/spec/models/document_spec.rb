...
describe Document do
  ...
  before(:each) do
    @valid_document=Document.new(:complete_path => "spec/test_dirs/indexed/basic/basic.pdf")
  end
  ...
  basic_pdf_attribute.each{|attribute,expected_value|
    it "should know its #{attribute}" do
      ...
      @basic_pdf=Document.new(:complete_path => 'spec/test_dirs/indexed/basic/basic.pdf')
      @basic_pdf.send(attribute).should == expected_value
    end
  }

  it "should know its content" do
    another_doc=Document.new(:complete_path => "spec/test_dirs/indexed/basic/plain.txt")
  ...
  it "should keep content cached" do
    ...
    some_doc=Document.new(:complete_path => filename)
  ...
  it "should know if its content can be extracted" do
    ...
    Document.new(:complete_path => "spec/test_dirs/indexed/others/ghjopdfg.xyz").should_not be_supported
  end

  it "should not be considered supported if binary" do
    Document.new(:complete_path => "spec/test_dirs/indexed/others/BIN_FILE_WITHOUT_EXTENSION").should_not be_supported
  end
  
  it "should know its language when enough content is available" do
    Document.new(:complete_path => "spec/test_dirs/indexed/lang/goethe").language.should == "de"
    Document.new(:complete_path => "spec/test_dirs/indexed/lang/shakespeare").language.should == "en"
    Document.new(:complete_path => "spec/test_dirs/indexed/lang/lorca").language.should == "es"
    Document.new(:complete_path => "spec/test_dirs/indexed/lang/hugo").language.should == "fr"
  end if Picolena::UseLanguageRecognition

  it "should not try to guess language when file is too small" do
    Document.new(:complete_path => "spec/test_dirs/indexed/basic/hello.rb").language.should be_nil
    Document.new(:complete_path => "spec/test_dirs/indexed/README").language.should be_nil
  end if Picolena::UseLanguageRecognition
  ...
  it "should know its icon_path if a thumbnail if available" do
    doc=Document.new(:complete_path => "spec/test_dirs/indexed/media/badminton.avi")
  ...
  it "should know its icon_path if an icon  if available for its mimetype" do
    doc=Document.new(:complete_path => "spec/test_dirs/indexed/others/xor.vee")
    doc.icon_path.should_not be_nil
    doc.icon_path.should == 'icons/insel.png'
  end

  it "should return nil as icon_path if no icon or thumbnail is available" do
    doc=Document.new(:complete_path => "spec/test_dirs/indexed/others/ghjopdfg.xyz")
    doc.icon_path.should be_nil
  end

  it "should not raise when asked for highlighted_cache even though content is empty" do
    doc=Document.new(:complete_path => "spec/test_dirs/indexed/others/nested/unreadable.pdf")
